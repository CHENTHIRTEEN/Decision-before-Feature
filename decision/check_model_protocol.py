...
def check_model_protocol(training_dir: Path | None = None) -> dict[str, Any]:
    ...
    expected_objectives = {"random_forest_regressor": "regression"}
    if objectives != expected_objectives:
        raise ValueError(f"Decision objective mapping changed: {objectives}")
    if [spec.model_name for spec in specs if spec.supports_utility_rmse] != ["random_forest_regressor"]:
        raise ValueError("continuous Utility RMSE must be defined for the active RF regressor")
    banned = [
        spec.model_name
        for spec in specs
    ...
    result: dict[str, Any] = {
        ...
        "predefined_threshold_mode": PREDEFINED_THRESHOLD_MODE,
...
def _check_training_artifacts(training_dir: Path) -> dict[str, Any]:
    ...
    if summary.get("threshold_modes") != ["zero", PREDEFINED_THRESHOLD_MODE]:
        raise ValueError("training summary threshold modes do not match the predefined protocol")
    ...
    if tuple(selection.sort_values("candidate_order")["model_name"].astype(str)) != ACTIVE_MODEL_NAMES:
        raise ValueError("model-selection table candidates do not match the predefined order")
    ...
    expected_pairs = {(name, mode) for name in ACTIVE_MODEL_NAMES for mode in ("zero", PREDEFINED_THRESHOLD_MODE)}
    ...
    oof_thresholds = thresholds[
        thresholds["threshold_mode"].astype(str) == PREDEFINED_THRESHOLD_MODE
    ]
    ...
    if not np.allclose(
        ...
        raise ValueError("threshold-neighborhood quantile does not match the predefined Q10 protocol")
    ...
    if set(folds["fold_role"].astype(str)) != expected_roles:
        raise ValueError("OOF fold table does not cover the three predefined fold roles")
    ...
    rf_regressor = all_validation[
        all_validation["model_name"].astype(str) == "random_forest_regressor"
    ]
    if len(rf_regressor) != 1 or not bool(rf_regressor["rmse_applicable"].iloc[0]) or not np.isfinite(float(rf_regressor["rmse"].iloc[0])):
        raise ValueError("Random Forest Regressor must report one finite all-validation continuous Utility RMSE")
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Check the fixed RF-regression nested landscape-family OOF Decision protocol."
    )
