...
run- and instance-level partitions in five and ten dimensions.  These protocols answer different questions from the present `cv_group_id = function_id` OOF design; they are antecedents for representation and warm-starting, not precedents for the present evaluation design.
...
\cite{cenikjSurveyFeaturesUsed2026}.  These observations motivate
function-ID-grouped out-of-fold selection and function-held-out evaluation;
they do not guarantee that the resulting gate transfers.  In the present
study, the BBOB-validation outcomes were inspected during earlier development,
so that split supplies an internal finite-set estimate rather than an
...
uses function-ID-grouped partitions and selects the Decision Model by policy utility
...
nested function-level evaluation.  The literature supports this formulation and its
design constraints; it does not supply evidence for the pending RQ1--RQ5
outcomes or for generalization across function IDs or benchmark suites.
