...
The primary acquisition decision is
\begin{equation}
  d_t=
  \begin{cases}
    0, & \text{choose the best available no-query policy},\\
    1, & \text{acquire }q\text{ and invoke its selector}.
  \end{cases}
  \label{eq:analysis-decision}
\end{equation}
The no-query side is represented operationally by the best available comparator among native Skip and full-budget Behavior-only, with Behavior-only serving as the main no-query policy in the theory and experiments.
...
regressor through the within-state target
\begin{equation}
  y_{s,a}
  =\ell_{s,a}-\ell_{s,a_0},
  \qquad
...
\begin{equation}
  \ell_{s,a}
  =\log_{10}\!\left[
    \min\!\left(\max(g_{s,a},g_{\min}),g_{\max}\right)
  \right].
  \label{eq:selector-target-loggap}
\end{equation}
This choice keeps the continuation action fixed at zero, makes negative values
mean that action $a$ improves on continue-current, and aligns the selector
with the decision rule used downstream.  The minimum over actions that were
actually run is called the \emph{best observed action}, not an oracle.
...
each selected path from the same decision state for the five-path protocol
...
\begin{align}
  query_operational_increment(S_t)
  &=(\ell_b-\ell_q)
  -\lambda_T\bigl(\log_{10}T_q-\log_{10}T_b\bigr)\\
  &=U_q^{\mathrm{joint}}(S_t)-U_b(S_t).
  \label{eq:operational-increment}
\end{align}
$query_operational_increment$ includes query evaluations, sample-best contribution, unequal
...
$U_q^{\mathrm{joint}}>0$ but $query_operational_increment\leq0$ in the corresponding population, the
...
query-budget outcomes for the five paths; their OOF-selected continuation-only gaps
define
\begin{equation}
...
The active protocol contains five paths: Skip, Behavior-only,
sampling-only, query-matched state-only, and full Query.  Their operational
decomposition provides the component increments $D_q$, $S_q$, and $A_q$ whose
sum equals $U_q^{\mathrm{joint}}$.  Neither comparison separately identifies
a causal contribution of the descriptors, because sampling, sample-best, FE,
runtime, and budget effects are entangled.  The five-path decomposition is
an interpretation tool; the binary label itself uses $query_operational_increment$ so that the
query is judged against the best available no-query policy.

The continuous joint utility is retained, but the binary label is aligned with
query acquisition against the best available no-query policy:
\begin{equation}
  y_q(S_t)=\mathbf{1}\!\left[query_operational_increment(S_t)>0\right],
  \label{eq:query-label}
\end{equation}
with \(query_operational_increment\) defined in Eq.~\eqref{eq:operational-increment}.  This choice means a state is positive only when the query path improves on the best available no-query comparator, not merely on native Skip.  The Decision input contains only information available before the query.  It excludes query descriptors, benchmark and algorithm identity, optimizer parameters, known-optimum gaps, action outcomes, and future states.
